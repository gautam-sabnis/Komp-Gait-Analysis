...
for (s in 1:length(Strains)){
        ...
        #Nose 
        nose <- read.csv(paste0('Data/Phase_dfs/tip_tail/',Strains[s],'.csv'), header = TRUE)
        ...
        invisible(lapply(seq(length(unique(nose$MouseID))), function(x) {
                ...
                tryCatch({
                        invisible(capture.output(out <- fdasrvf::time_warping(tmp$f,tmp$time, method = 'median',showplot=FALSE)))
...
write.csv(x = df.nose, file = 'Output/base_tail.csv')
